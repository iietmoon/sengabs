...
class Trail {
	...
	public function default_labels() {
		...
		$labels = [
			'browse'              => __( 'Browse:', 'stax-addons-for-elementor' ),
			'home'                => __( 'Home', 'stax-addons-for-elementor' ),
			'error_404'           => __( '404 Not Found', 'stax-addons-for-elementor' ),
			'archives'            => __( 'Archives', 'stax-addons-for-elementor' ),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => __( 'Search results for &#8220;%s&#8221;', 'stax-addons-for-elementor' ),
			/* Translators: %s is the page number. */
			'paged'               => __( 'Page %s', 'stax-addons-for-elementor' ),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      => __( 'Minute %s', 'stax-addons-for-elementor' ),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => __( 'Week %s', 'stax-addons-for-elementor' ),
	...
	public function do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'stax-addons-for-elementor' ) ) );
		}
	}
	...
	public function do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'stax-addons-for-elementor' ) ) );
		}
	}
	...
	public function do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'stax-addons-for-elementor' ) ) );
		}
	}
	...
	public function do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'stax-addons-for-elementor' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'stax-addons-for-elementor' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'stax-addons-for-elementor' ) ) );
	...
	public function do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'stax-addons-for-elementor' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'], get_the_time( _x( 'W', 'weekly archives date format', 'stax-addons-for-elementor' ) ) );
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'stax-addons-for-elementor' ) ) ) . '</a>';
				} /* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'stax-addons-for-elementor' ) ) ) . '</a>';
				} /* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag ) {
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'stax-addons-for-elementor' ) ) ) . '</a>';
